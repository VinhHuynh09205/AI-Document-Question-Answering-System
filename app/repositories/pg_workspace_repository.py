...
class PgWorkspaceRepository(IWorkspaceRepository):
    ...
    def add_document(self, username: str, chat_id: str, original_name: str, stored_path: str) -> StoredDocument:
        ...
        try:
            ...
            conn.close()
        return StoredDocument(document_id=document_id, chat_id=chat_id, username=username,
                             original_name=original_name, stored_path=stored_path, created_at=created_at)
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    FROM documents WHERE username = %s AND chat_id = %s ORDER BY created_at ASC
                """, (username, chat_id))
                rows = cur.fetchall()
        finally:
            conn.close()
        return [
            StoredDocument(document_id=row[0], chat_id=row[1], username=row[2],
    ...
    def rename_document(self, username: str, document_id: str, new_name: str) -> StoredDocument | None:
        ...
        if row is None:
            return None
        return StoredDocument(document_id=row[0], chat_id=row[1], username=row[2],
                             original_name=row[3], stored_path=row[4], created_at=self._to_iso(row[5]))
